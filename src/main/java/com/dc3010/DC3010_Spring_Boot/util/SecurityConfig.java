...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		http.csrf().disable();
		http.authorizeHttpRequests(authorize -> authorize.dispatcherTypeMatchers(DispatcherType.FORWARD,DispatcherType.ERROR).permitAll().requestMatchers("/css/**", "/images/**", "/js/**","/resources/**").permitAll()
				.requestMatchers("/dashboard", "/profile", "/update/user/details", "/update/user/password").hasAnyRole("USER","RM" ,"ADMIN").requestMatchers("/create", "/register").hasRole("ADMIN").requestMatchers("/manage","/create/project").hasAnyRole("ADMIN", "RM"))
