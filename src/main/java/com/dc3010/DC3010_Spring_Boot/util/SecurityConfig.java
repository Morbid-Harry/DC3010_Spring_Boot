...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		http.authorizeHttpRequests(authorize -> authorize.dispatcherTypeMatchers(DispatcherType.FORWARD,DispatcherType.ERROR, DispatcherType.ASYNC).permitAll().requestMatchers("/css/**", "/images/**", "/js/**","/resources/**").permitAll()
				.requestMatchers("/dashboard", "/profile", "/update/user/details", "/update/user/password", "/findOne/{id}", "/findOne/**").hasAnyRole("USER","RM" ,"ADMIN").requestMatchers("/create", "/register").hasRole("ADMIN").requestMatchers("/manage","/create/project").hasAnyRole("ADMIN", "RM"))
